...
@pytest.mark.parametrize("planet_id", [PlanetID.ODIN_INTERNAL, PlanetID.HEIMDALL_INTERNAL, PlanetID.THOR_INTERNAL])
@pytest.mark.parametrize("pass_type",
...
def test_current_season(sess, planet_id, pass_type):
    ...
    with add_test_data(sess, *[season_data]) as test_data:
        ...
        resp = tc.get("/api/season-pass/current",
                      params={"planet_id": planet_id.value.decode(),
