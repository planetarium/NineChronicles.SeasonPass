...
def upgrade() -> None:
    ...
    op.execute(
        "UPDATE block set pass_type='COURAGE_PASS'"
    )  # all the prev. block data are for courage pass
    op.alter_column("block", "pass_type", nullable=False)
    op.drop_index("idx_block_planet_index", table_name="block")
    op.create_index(
        "idx_block_planet_pass_type_index",
        "block",
        ["planet_id", "pass_type", "index"],
        unique=False,
    )
